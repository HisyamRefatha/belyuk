                ...
                foreach (var entityType in model.GetScaffoldEntityTypes(_options.Value))
                {
                    ...
                    if (create_code)
                    {
                        ...
                        if (is_master)
                        {
                            ...
                            string primary_type = list_properties.Where(w => w.IsPrimaryKey()).Select(s => ParseType(s.ClrType)).FirstOrDefault()!;
                            ...
                            string primary_name = list_properties.Where(w => w.IsPrimaryKey()).Select(s => s.Name).FirstOrDefault()!;

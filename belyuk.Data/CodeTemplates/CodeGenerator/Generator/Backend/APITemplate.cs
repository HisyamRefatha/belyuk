...
namespace belyuk.Data.CodeTemplates.CodeGenerator.Generator
{
        ...
        public static (bool success, string key, object val) GenerateAPICQRS(string current_namespace, IModel model, Microsoft.Extensions.Options.IOptions<HandlebarsScaffoldingOptions> _options, List<SettingExcludeTableCodeGeneratorObject> exclude)
        {
            ...
            using (sb.Indent())
            {
                foreach (var entityType in model.GetScaffoldEntityTypes(_options.Value))
                {
                    ...
                    {
                        var exclude_table = exclude.Where(w => w.TableName.ToLower() == entityType.Name.ToLower()).FirstOrDefault();
                    ...
                    if (create_code)
                    {
                        ...
                        string primary_type = list_properties.Where(w => w.IsPrimaryKey() == true).Select(s => ParseType(s.ClrType)).FirstOrDefault()!;
                        if (string.IsNullOrWhiteSpace(primary_type))
                            primary_type = list_properties.Where(w => w.Name.ToLower() == ("id")).Select(s => ParseType(s.ClrType)).FirstOrDefault()!;
